...
class PHPImage {
    ...
    protected function initialiseCanvas($width, $height, $resource='img'){
        ...
        imagefilledrectangle($this->$resource, 0, 0, $this->width, $this->height, imagecolorallocatealpha($this->$resource, 255, 255, 255, 127));
    ...
    public function show(){
        switch($this->type){
            ...
            case IMAGETYPE_PNG:
                ...
                imagepng($this->img, null, $this->quality, PNG_ALL_FILTERS);
                break;
            default:
        ...
        if (is_writable(dirname($path))) {
            switch($this->type){
                case IMAGETYPE_GIF:
                    ...
                    break;
                case IMAGETYPE_PNG:
                    imagepng($this->img, $path, $this->quality, PNG_ALL_FILTERS);
                    break;
                default:
    ...
    public function Trim(){
        $height = $this->height;
        $width = $this->width;
        $top = ['remove' => true, 'color' => 0xFFFFFF, 'tolerance' => (0xFFFFFF-0xCCCCCC)];
        $bottom = ['remove' => true, 'color' => 0xFFFFFF, 'tolerance' => (0xFFFFFF-0xCCCCCC)];
        $left = ['remove' => true, 'color' => 0xFFFFFF, 'tolerance' => (0xFFFFFF-0xCCCCCC)];
        $right = ['remove' => true, 'color' => 0xFFFFFF, 'tolerance' => (0xFFFFFF-0xCCCCCC)];
        $margin = '2';
        $crop = ['top' => 0, 'bottom' => $height, 'left' => 0, 'right' => $width];
        if (isset ($top['remove']) && $top['remove'] === true && isset ($top['color']) && $top['color'] != ''){
            $found = false;
            for ($y = 0; $y < $height; $y++){
                for ($x = 0; $x < $width; $x++){
                    $pixel = imagecolorat ($this->img, $x, $y);
                    if (($top['color'] < ($pixel - $top['tolerance'])) || ($top['color'] > ($pixel + $top['tolerance']))){
                        $crop['top'] = $y;
                        $found = true;
                        break;
                    }
        ...
        if (isset ($bottom['remove']) && $bottom['remove'] === true && isset ($bottom['color']) && $bottom['color'] != ''){
            $found = false;
            for ($y = ($height - 1); $y >= 0; $y--){
                for ($x = 0; $x < $width; $x++){
                    $pixel = imagecolorat ($this->img, $x, $y);
                    if (($bottom['color'] < ($pixel - $bottom['tolerance'])) || ($bottom['color'] > ($pixel + $bottom['tolerance']))){
                        $crop['bottom'] = $y + 1;
                        $found = true;
                        break;
                    }
        ...
        if (isset ($left['remove']) && $left['remove'] === true && isset ($left['color']) && $left['color'] != ''){
            $found = false;
            for ($x = 0; $x < $width; $x++){
                for ($y = 0; $y < $height; $y++){
            $pixel = imagecolorat ($this->img, $x, $y);
            if (($left['color'] < ($pixel - $left['tolerance'])) || ($left['color'] > ($pixel + $left['tolerance']))){
                $crop['left'] = $x;
                $found = true;
                break;
            }
        ...
        if (isset ($right['remove']) && $right['remove'] === true && isset ($right['color']) && $right['color'] != ''){
            $found = false;
            for ($x = ($width - 1); $x >= 0; $x--){
                for ($y = 0; $y < $height; $y++){
            $pixel = imagecolorat ($this->img, $x, $y);
            if (($right['color'] < ($pixel - $right['tolerance'])) || ($right['color'] > ($pixel + $right['tolerance']))){
                $crop['right'] = $x + 1;
                $found = true;
                break;
            }
        ...
        $newres = ['x' => ($crop['right'] - $crop['left']), 'y' => ($crop['bottom'] - $crop['top'])];
        $tmp = $this->img;
        $this->initialiseCanvas($newres['x']+($margin*2), $newres['y']+($margin*2));
        imagecopy ($this->img, $tmp, $margin, $margin, $crop['left'], $crop['top'], $newres['x'], $newres['y']);
        imagedestroy($tmp);
        $this->afterUpdate();
        return $this;
    }
    ...
    public function applyFilter($filterType, $arg1 = null, $arg2 = null, $arg3 = null, $arg4 = null)
    {
        if ($filterType == IMG_FILTER_COLORIZE) {
            imagefilter($this->img, $filterType, $arg1, $arg2, $arg3, $arg4);
    ...
    /**
     * Identify image grayscale
     *
     * @return bool
     */
    public function isGrayscale() {
        $margin = 2;
        imagetruecolortopalette($this->img, false, 255 * 255 * 255);
        for ($j = 0; $j < imagecolorstotal($this->img); $j++) {
            $color[$j] = imagecolorsforindex($this->img, $j);
            $grayscale = round($color[$j]['red'] * 0.2125 + $color[$j]['blue'] * 0.7154 + $color[$j]['green'] * 0.0721);
            if ($color[$j]['red'] - $grayscale <= $margin || $grayscale - $color[$j]['red'] >= $margin) {
                if ($color[$j]['blue'] - $grayscale <= $margin || $grayscale - $color[$j]['blue'] >= $margin) {
                    if ($color[$j]['green'] - $grayscale <= $margin || $grayscale - $color[$j]['green'] >= $margin) {
                        $gray = true;
                    } else {
                        $gray = false;
                    }
     ...
     * @return $this
     */
    public function png_optimization(){
